...
// Recipe createdRecipe = recipeServiceImpl.createRecipeToUser(user.getId(),
// recipe);

// // 3c. assert the results
// assertEquals(recipe, createdRecipe, "Recipe should be the same as create
// recipe");

// // 3d. verify that the the method is only calling the recipe repo and saving
// 1
// // time
...
// // but throws an error
// when(recipeRepo.save(recipe)).thenThrow(new RuntimeException("Database
// connection error"));
...
// Integer recipeId = 1;
// // mocking recipeRepo behaviour by finding the recipe base on the recipe id
// and
// // return optional
...
// recipeServiceImpl.getOneRecipe(recipeId));
// }
...
// .description("A simple yet flavorful Dijon mustard salmon baked to
// perfection.")
...
// "Brush the salmon fillets with the mustard mixture and season with salt and
// pepper.",
// "Bake for 12-15 minutes until salmon is cooked through."))
// .build();

// // Mock saving the repo then return the updated recipe (this will be called
// when
// // the updateOne recipe is called)
// when(recipeRepo.save(recipe)).thenReturn(recipeToUpdate);

// // call the updateOneRecipe method
// Recipe updatedRecipe = recipeServiceImpl.updateOneRecipe(recipeId,
...
// when(recipeRepo.findById(recipeId)).thenReturn(Optional.empty());
// assertThrows(RecipeNotFoundException.class, () ->
// recipeServiceImpl.updateOneRecipe(recipeId, recipe, userId));
// }
